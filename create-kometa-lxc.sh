...
elif [[ "$INSTALL_METHOD" == "linux" ]]; then
  INSTALL_SCRIPT_URL="https://raw.githubusercontent.com/Mr-R-b0t/tidarr-lxc/main/install/kometa-linux-install.sh"
else
  echo -e "${RD}Invalid installation method. Use 'docker' or 'linux'${CL}"
  exit 1
fi
...
EOF
  echo -e "${YW}Kometa LXC Container Creator (${INSTALL_METHOD})${CL}\n"
}
...
echo -e "  Method:   ${GN}$INSTALL_METHOD${CL}"
echo ""
...
echo -e "  Edit config at: /opt/kometa/config/config.yml"
echo ""
if [[ "$INSTALL_METHOD" == "docker" ]]; then
  echo -e "${YW}Useful commands:${CL}"
  echo -e "  pct exec $CTID -- docker ps"
  echo -e "  pct exec $CTID -- bash -c 'cd /opt/kometa && docker compose logs -f'"
else
  echo -e "${YW}Useful commands:${CL}"
  echo -e "  pct exec $CTID -- systemctl status kometa"
  echo -e "  pct exec $CTID -- journalctl -u kometa -f"
fi
