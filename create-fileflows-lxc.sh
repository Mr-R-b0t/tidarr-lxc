...
INSTALL_SCRIPT_URL="https://raw.githubusercontent.com/Mr-R-b0t/tidarr-lxc/main/install/fileflows.sh"
...
EOF
  echo -e "${YW}FileFlows LXC Container Creator${CL}\n"
}
...
msg_info "Configuring container for FileFlows"
# Add FileFlows-friendly settings to LXC config
cat >> /etc/pve/lxc/${CTID}.conf <<EOF
lxc.apparmor.profile: unconfined
lxc.cap.drop: 
EOF
msg_ok "Configured container for FileFlows"
...
echo -e "  ${GN}http://${CONTAINER_IP}:5000${CL}"
echo ""
echo -e "${YW}Useful commands:${CL}"
echo -e "  pct exec $CTID -- systemctl status fileflows-node"
echo -e "  pct exec $CTID -- systemctl logs fileflows-node -f"
